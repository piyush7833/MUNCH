...
const Profile = async () => {
	...
	const shopOwnerData: responseShopOwnerType | null | undefined =
		user?.shopOwner?.[0];
		...
		<div>
			<div className="main gap-4 hideScrollBar">
				<UserProfile
					...
					shopOwnerExtracedData={shopOwnerExtracedData}
				/>
				{userData.role === "ShopOwner" &&
					user?.shopOwner?.[0].verified && (
						<UserShops shops={shopData} userId={userData.id!} />
					)}
				{userData.role === "ShopOwner" &&
					!user?.shopOwner?.[0].verified && (
						<div>
							ShopOwner details are updated and yet to be verified
						</div>
					)}
			</div>
		</div>
